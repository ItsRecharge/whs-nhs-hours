...
/** Reads the one-time share-link cookie for `path`, shows it with a copy button, clears it. */
export function ShareLinkReveal({ path }: { path: string }) {
  ...
  useEffect(() => {
    const match = document.cookie
      .split("; ")
      .find((c) => c.startsWith("nhs_last_share="));
    if (match) {
      setLink(decodeURIComponent(match.split("=").slice(1).join("=")));
      document.cookie = `nhs_last_share=; path=${path}; max-age=0`;
    }
  }, [path]);
  ...
  return (
    <div className="rounded-xl border border-green-300 bg-green-50 p-4">
      <p className="mb-2 text-sm font-semibold text-green-800">
        New share link (copy it now — it won&apos;t be shown again):
      </p>
